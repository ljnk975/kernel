...
from org_rocks_rolls.constants import ROCKSROLLS_FILE_PATH

log = logging.getLogger(__name__)


class RocksRollsConfigurationTask(Task):
    """The RocksRolls configuration task.
...
class RocksRollsInstallationTask(Task):
    """The RocksRolls installation task.
    ...
    @property
    def name(self):
        return "Install RocksRolls"

    def run(self):
        """The run method performs the actual work."""
        log.info("Running installation task.")
        rocksrolls_file_path = normpath(joinpath(self._sysroot, ROCKSROLLS_FILE_PATH))
        log.debug("Writing rocksrolls file to: %s", rocksrolls_file_path)
        ...
        with open(rocksrolls_file_path, "w") as rocksrolls_file:
            for line in iterator:
                rocksrolls_file.write(line)
